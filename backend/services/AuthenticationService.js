...
class userAuthenticationService{
            ...
            try {
                ...
                if (isUserExist.length > 0) {
                    ...
                    const insertResult = await execute(registerQuery, [
                        ...
                        currentDate,
                        currentDate,
                        true // Assuming isActive is always true for new registrations
                    ]);
    ...
    async partnerActive(id){
        let query = "select email, password from partners where id = ?"
        let result = await execute(query, [id])
        let decryptedPassword = await decryptedData(result[0].password)
        await sendMailToUser(result[0].email, decryptedPassword)
        query = 'update partners set is_active = ? where id = ?'
        result = await execute(query, [1, id])
